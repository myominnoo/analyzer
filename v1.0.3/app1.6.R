...
rename_server <- function(id, data, launch) {
  moduleServer(id, function(input, output, session) {
    ...
    output$rename_table <- rhandsontable::renderRHandsontable({
      ...
      rhandsontable::rhandsontable(data.frame(old = names(data()), new = ""),
      ...
      if (inherits(res_update, "try-error")) {
        ...
        if (num_changed > 0) {
          insert_alert(
            selector = ns("update"),
            status = "success",
            tags$b(phosphoricons::ph("check"), sprintf(
              "%s variables successfully renamed!", num_changed
            ))
          )
          env$updated_data <- df
          ## log code 
          ...
          insert_error("No changes!", "update", "warning")
        }
      }
    })
...
ui <- fluidPage(
  ...
  wellPanel(
    ...
    div(actionLink("show_active_dataset", btn_lbl_icon("table", "View active dataset"))),
...
server <- function(input, output, session) {
  ...
  # server - show active dataset --------------------------------------------
  observeEvent(input$show_active_dataset, {
    req(env$data)
    datamods::show_data(env$data, title = "Active Dataset", type = "modal", 
                        width = "100%")
  })
